...
from .utils import prettify, sample_xyzs, rotation_matrix, add_title_to_img
from .ik import solve_ik
from .transforms import rpy2r, r2rpy

# 绝对导入
# from experiments.sim.mujoco_env.mujoco_parser import MuJoCoParserClass
# from experiments.sim.mujoco_env.utils import prettify, sample_xyzs, rotation_matrix, add_title_to_img
# from experiments.sim.mujoco_env.ik import solve_ik
# from experiments.sim.mujoco_env.transforms import rpy2r, r2rpy
import os
import copy
import glfw


class MobileSingleManipulatorEnv:
    def __init__(
        ...
        """
        # Load the xml file
        self.env = MuJoCoParserClass(name="pickandplace", rel_xml_path=xml_path)
    ...
    def step(self, action):
        ...
        # case1 末端执行器位姿控制 ——— 默认模式！
        if self.action_type == "eef_pose":
            q = self.env.get_qpos_joints(joint_names=self.joint_names)

            # 累加末端位姿变化
            self.p0 += action[:3]  # 位置累加：p0 = p0 + [dx, dy, dz]
            self.R0 = self.R0.dot(rpy2r(action[3:6]))  # 姿态累乘：R0 = R0 × R_delta

            # 🔑 关键步骤：通过逆运动学(IK)求解关节角度
            q, ik_err_stack, ik_info = solve_ik(
                env=self.env,
                joint_names_for_ik=self.joint_names,
                body_name_trgt="tcp_link",  # 目标身体是 TCP 链接点
                q_init=q,  # 从当前关节角度开始迭代
                p_trgt=self.p0,  # 目标位置
                R_trgt=self.R0,  # 目标姿态
                max_ik_tick=50,  # 最大迭代次数
                ik_stepsize=1.0,  # IK步长
                ik_eps=1e-2,  # 收敛精度
                ik_th=np.radians(5.0),  # 角度阈值5度
                render=False,
    ...
    def teleop_robot(self):
        ...
        dpos = np.zeros(3)  # 位置变化量 [dx, dy, dz]
        drot = np.eye(3)  # 旋转矩阵（3x3单位矩阵）
        ...
        # * 最终组合成7维动作向量：[dx, dy, dz, droll, dpitch, dyaw, gripper]，这个 action 向量的前 6 维表示的是末端执行器（TCP）的位姿增量，而不是关节角度！
    ...
    def check_success(self):
        ...
        if (
            ...
            and self.env.get_qpos_joint("rh_r1") < 0.1
        ):
