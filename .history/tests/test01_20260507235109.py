...
def print_model_attr(model):
    ...
    for i in range(model.ngeom):
        if (
            ...
            or model.geom(i).name == "lab_table_body" or model.geom(i).name == "lab_table_visual"
        ):
            continue
...
def main():
    ...
    # 获取 base_link 的索引
    # base_id = model.body("gen3/base_link").id

    # 设置位置
    # data.qpos[0:3] = [-3.0, 0, 0]  # joint_x, joint_y, joint_th
    # 设置绕 Z 轴旋转 90 度的四元数
    # theta = np.pi / 2  # 90度
    # quat_z = np.array([np.cos(theta/2), 0, 0, np.sin(theta/2)])
    # data.qpos[3:7] = quat_z
