...
def print_info(model):
    ...
    # case1 查看模型的 geom 属性
    print(f"# 模型的 geom 数量：", model.ngeom)
    ...
    print(f"-- 模型的 body 名称为: ", [model.body(i).name for i in range(model.nbody)])
    # ["world", "base_link"]
    ...
    print(f"模型的 actuator 数量：", model.nu)  # 1 - 执行器数量

    # case4 查看关节属性
    print(f"模型的 joint 数量：", model.njnt)
    joint_idx = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_idx)
...
def main():
    ...
    xml_path = "/home/robot/uni-wbc-vla/experiments/sim/assets/arenas/pickandplace/pickandplace.xml"
