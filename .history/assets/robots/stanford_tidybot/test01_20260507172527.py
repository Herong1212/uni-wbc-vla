...
def print_model_attr(model):
    ...
    print(f"模型的 geom 数量：", model.nq)  # 3 - 广义坐标数量（位置自由度）
    print(f"模型的 geom 数量：", model.nv)  # 3 - 广义速度数量（速度自由度）
    print(f"模型的 geom 数量：", model.nlight)  # 1 - 光源数量
    print(f"模型的 geom 数量：", model.ncam)  # 1 - 相机数量
