...
# 将项目根目录加入 sys.path，无论从哪个目录运行脚本都能正确找到模块
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", ".."))
from UniRobot.sim.mujoco_env.tidybot_env import (
    TidybotEnv,
)
from UniRobot.sim.mujoco_env.y_env import SimpleEnv

# from lerobot.datasets.lerobot_dataset import LeRobotDataset # 这是 v0.1.1 版本的 lerobot！
...
REPO_NAME = "omy_pnp"  # 数据集仓库名称
NUM_DEMO = 3  # 要收集的演示数量（3个完整任务回合）
ROOT = "./outputs/datasets"  # 数据集保存的根目录路径

TASK_NAME = "Pick up the cola bottle and put it in the box."
# xml_path = "experiments/sim/assets/arenas/pickandplace/scene.xml"
xml_path = (
    "src/UniRobot/sim/mujoco_env/assets/arenas/scanned_objects/example_scene_y.xml"
)

# * 定义环境
PnPEnv = TidybotEnv(xml_path, seed=SEED, state_type="joint_angle")
