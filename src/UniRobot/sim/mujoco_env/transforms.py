...
def r2quat(R):
    """
    Convert Rotation Matrix to Quaternion.  See rotation.py for notes https://gist.github.com/machinaut/dab261b78ac19641e91c6490fb9faa96
    """
...
def pr2t(p, R):
    ...
    """
    p0 = p.ravel()  # flatten
    T = np.block([[R, p0[:, np.newaxis]], [np.zeros(3), 1]])
    return T
...
def get_rotation_matrix_from_two_points(p_fr, p_to):
    ...
    if np.linalg.norm(v) == 0:
        ...
        R = (
            np.eye(3, 3)
            + S
            + S @ S * (1 - np.dot(p_a, p_b)) / (np.linalg.norm(v) * np.linalg.norm(v))
        )
    return R
