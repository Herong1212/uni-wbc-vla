...
class MuJoCoParserClass(object):
    ...
    def __init__(
        self,
        name=None,  # 为仿真环境指定一个名称
        rel_xml_path=None,  # MuJoCo 场景描述文件的相对路径
        xml_string=None,  # 直接传入 XML 格式的字符串内容，一般不用
        assets=None,
        verbose=True,  # 控制是否打印详细的初始化信息
    ):
        """
        Initialize MuJoCo parser
        """
        # 环境标识符，用户手动指定或从 XML 的 <model name="..."> 标签提取
        self.name = name
        self.rel_xml_path = rel_xml_path
        self.xml_string = xml_string
        self.assets = assets
        self.verbose = verbose  # 控制后续操作中是否输出详细信息

        # Constants
        # int，仿真步数计数器，记录从环境创建以来执行的总步数
        self.tick = 0
        # int，渲染帧计数器，独立于仿真步数的渲染计数，用于控制渲染频率（如每 N 步渲染一次），每次调用 render() 时递增
        self.render_tick = 0
        # bool，标记是否启用 MuJoCo 可视化查看器。True: 已调用 init_viewer()，可以进行实时可视化；False: 无头模式（headless），仅进行仿真计算
        ...
        # Tic-toc 性能计时器，用于测量仿真步进、IK 求解等操作的耗时、性能分析和瓶颈定位
        self.tt = TicTocClass(name="env:[%s]" % (self.name))
        # tt.tic() # 开始计时
        # ... 执行操作 ...
        # tt.toc() # 获取 elapsed time

        # Monitor size
        self.monitor_width, self.monitor_height = get_monitor_size()  # 5120,1600

        # 如果 verbose=True，则在初始化时调用 print_info()，输出模型统计信息（关节数、身体数、传感器数等）
        if self.verbose:
            self.print_info()
    ...
    def _parse_xml(self):
        ...
        if self.xml_string is not None:
            ...
            self.model = mujoco.MjModel.from_xml_string(
                xml=self.xml_string, assets=self.assets
            )

        # Parse xml model name
        parsed_strings = [s for s in self.model.names.split(b"\x00") if s]  # list
        parsed_strings = [s.decode("utf-8") for s in parsed_strings]
        self.model_name = parsed_strings[0]  # 'Scanned Objects'，从 XML 提取的模型名称

        self.data = mujoco.MjData(self.model)
        self.dt = self.model.opt.timestep  # 仿真步长为 0.002
        self.HZ = int(1 / self.dt)  # 500

        # ps Integrator（积分器，此处使用的是 "implicitfast" 积分器） (https://mujoco.readthedocs.io/en/latest/APIreference/APItypes.html#mjtintegrator)
        ...
        self.n_qacc = (
            self.model.nv
        )  # number of accelerations (dimension of tangent space)

        # Geometry
        self.n_geom = self.model.ngeom  # number of geometries
        self.geom_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_GEOM, geom_idx)
        ...
        self.mesh_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_MESH, mesh_idx)
        ...
        self.body_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_BODY, body_idx)
        ...
        self.dof_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_DOF, dof_idx)
        ...
        self.joint_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_idx)
        ...
        # Free joint
        self.free_joint_idxs = np.where(self.joint_types == mujoco.mjtJoint.mjJNT_FREE)[
            0
        ].astype(np.int32)
        self.free_joint_names = [
            self.joint_names[joint_idx] for joint_idx in self.free_joint_idxs
        ]
        ...
        self.rev_joint_names = [
            self.joint_names[joint_idx] for joint_idx in self.rev_joint_idxs
        ]
        ...
        self.pri_joint_names = [
            self.joint_names[joint_idx] for joint_idx in self.pri_joint_idxs
        ]
        ...
        self.rev_pri_joint_idxs = np.concatenate(
            [self.rev_joint_idxs, self.pri_joint_idxs]
        )
        self.rev_pri_joint_names = self.rev_joint_names + self.pri_joint_names
        self.rev_pri_joint_mins = np.concatenate(
            [self.rev_joint_mins, self.pri_joint_mins]
        )
        self.rev_pri_joint_maxs = np.concatenate(
            [self.rev_joint_maxs, self.pri_joint_maxs]
        )
        ...
        self.ctrl_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_ACTUATOR, ctrl_idx)
        ...
        self.cam_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_CAMERA, cam_idx)
        ...
        self.sensor_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_SENSOR, sensor_idx)
        ...
        self.site_names = [
            mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_SITE, site_idx)
    ...
    def print_info(self):
        ...
        print(
            "# n_qpos:[%d] n_qvel:[%d] n_qacc:[%d] n_ctrl:[%d]"
            % (self.n_qpos, self.n_qvel, self.n_qacc, self.n_ctrl)
        )
        ...
        for body_idx, body_name in enumerate(self.body_names):
            ...
            print(
                " [%d/%d] [%s] mass:[%.2f]kg"
                % (body_idx, self.n_body, body_name, body_mass)
            )
        ...
        # ps 打印 joints 信息，包括关节 id、关节数量、关节名字、关节旋转轴
        print("")
        ...
        for joint_idx, joint_name in enumerate(self.joint_names):
            print(
                ...
                % (
                    ...
                    self.n_joint,
                    joint_name,
                    self.model.joint(joint_idx).axis,
                )
            )
        # ps 打印关节类型、关节范围
        ...
        for idx, free_joint_name in enumerate(self.free_joint_names):
            body_name_attached = self.body_names[
                self.model.joint(self.free_joint_idxs[idx]).bodyid[0]
            ]
            print(
                " [%d/%d] [%s] body_name_attached:[%s]"
        ...
        for idx, rev_joint_name in enumerate(self.rev_joint_names):
            print(
                ...
                % (
                    ...
                    self.n_rev_joint,
                    rev_joint_name,
                    self.rev_joint_mins[idx],
                    self.rev_joint_maxs[idx],
                )
            )
        ...
        for idx, pri_joint_name in enumerate(self.pri_joint_names):
            print(
                ...
                % (
                    ...
                    self.n_pri_joint,
                    pri_joint_name,
                    self.pri_joint_mins[idx],
                    self.pri_joint_maxs[idx],
                )
            )
        ...
        for idx, ctrl_name in enumerate(self.ctrl_names):
            print(
                " [%d/%d] [%s] range:[%.3f]~[%.3f] gear:[%.2f] type:[%s]"
                % (
                    idx,
                    self.n_ctrl,
                    ctrl_name,
    ...
    def print_body_joint_info(self):
        ...
        for body_idx in range(self.n_body):
            ...
            print(
                "[%2d/%d] body_name:[%s] parent_body_name:[%s]"
                % (
                    body_idx,
                    self.n_body,
                    colored(body_name, "green"),
                    colored(parent_body_name, "green"),
                )
            )
            print(
                " body p_offset:[%.2f,%.2f,%.2f] quat_offset:[%.2f,%.2f,%.2f,%.2f]"
                % (
                    p_body_offset[0],
                    p_body_offset[1],
                    p_body_offset[2],
            ...
            if n_joint == 0:  # fixed joint
                ...
                print(
                    " joint_name:[%s] n_joint:%s type:[%s]"
                    % (
                        colored(joint_name, "green"),
                        n_joint,
                        colored("%s" % (joint_type), "green"),
                    )
                )
                print(
                    " joint p_offset:[%.2f,%.2f,%.2f] axis:[%.1f,%.1f,%.1f]"
                    % (
                        p_joint_offset[0],
                        p_joint_offset[1],
                        p_joint_offset[2],
                ...
                print(
                    " n_joint:%s (%s)" % (n_joint, colored("composite joints", "red"))
                )
            print("")
    ...
    def reset(self, step=True):
        ...
        Reset
        """
        time.sleep(1e-3)  # 添加 1 毫秒的延迟
        mujoco.mj_resetData(self.model, self.data)  # * 将仿真数据重置为初始状态
        ...
        # Reset ticks
        self.tick = 0  # 仿真步数归零
        self.render_tick = 0  # 渲染帧数归零

        # Reset wall time
        self.init_sim_time = self.data.time  # 仿真时间起点
        self.init_wall_time = time.time()  # 真实世界时间起点
        self.accum_wall_time = 0.0  # 累积墙钟时间
        self.last_wall_update = time.time()  # 最后更新时间

        # Others
        self.xyz_left_double_click = None  # 重置左键双击位置
        self.xyz_right_double_click = None  # 重置右键双击位置

        # Print
        if self.verbose:
            print("env:[%s] reset" % (self.name))
    ...
    def init_viewer(
        ...
        # Print
        if self.verbose:
            print("env:[%s] initalize viewer" % (self.name))
    ...
    def set_viewer(
        ...
        # Geom group
        if geomgroup_0 is not None:
            self.viewer.vopt.geomgroup[0] = geomgroup_0
        if geomgroup_1 is not None:
            self.viewer.vopt.geomgroup[1] = geomgroup_1
        if geomgroup_2 is not None:
            self.viewer.vopt.geomgroup[2] = geomgroup_2
        if geomgroup_3 is not None:
            self.viewer.vopt.geomgroup[3] = geomgroup_3
        if geomgroup_4 is not None:
            self.viewer.vopt.geomgroup[4] = geomgroup_4
        if geomgroup_5 is not None:
            self.viewer.vopt.geomgroup[5] = geomgroup_5
        # Skybox
    ...
    def get_viewer_cam_info(self, verbose=False):
        ...
        if verbose:
            print(
                "azimuth:[%.2f] distance:[%.2f] elevation:[%.2f] lookat:%s]"
    ...
    def forward(self, q=None, joint_idxs=None, joint_names=None, increase_tick=True):
        ...
        if q is not None:
            ...
            ):  # if 'joint_names' is not None, it override 'joint_idxs'
    ...
    def set_pR_base_body(
        self, body_name="base", p=np.array([0, 0, 0]), R=np.eye(3), T=None
    ):
        """
    ...
    def set_pR_body(
        self, body_name="base", p=np.array([0, 0, 0]), R=np.eye(3), forward=True
    ):
        """
    ...
    def set_T_body(
        self, body_name="base", p=np.array([0, 0, 0]), R=np.eye(3), T=None, forward=True
    ):
        """
        Set both the position and orientation of a specified body.
    ...
    def set_geom_color(
        ...
        for idx, body_name in enumerate(body_names_to_color):  # for all bodies
            ...
            geom_idxs = [
                idx for idx, val in enumerate(self.model.geom_bodyid) if val == body_idx
            ]
    ...
    def set_geom_alpha(self, alpha=1.0, body_names_to_exclude=["world"]):
        ...
        for g_idx in range(self.n_geom):  # for each geom
            ...
            if body_name in body_names_to_exclude:
                continue  # exclude certain bodies
            # Change geom alpha
    ...
    def grab_rgbd_img(self):
        ...
        depth_img = np.zeros(
            (self.viewer.viewport.height, self.viewer.viewport.width, 1),
            dtype=np.float32,
        )
    ...
    def get_pcd_from_depth_img(self, depth_img, fovy=45):
        ...
        xyzone_transpose = np.vstack(
            (xyz_transpose, np.ones((1, xyz_transpose.shape[1])))
        )  # [4 x N]
    ...
    def get_egocentric_rgb(
        ...
        if restore_view:
            ...
            viewer_azimuth, viewer_distance, viewer_elevation, viewer_lookat = (
                self.get_viewer_cam_info()
            )
    ...
    def get_egocentric_rgbd_pcd(
        ...
        if rsz_rate_for_pcd is not None:
            ...
            depth_img_rsz = cv2.resize(
                depth_img, (w_rsz, h_rsz), interpolation=cv2.INTER_NEAREST
            )
        else:
        ...
        # Get PCD
        if fovy is None:
            if len(self.model.cam_fovy) == 0:
                fovy = 45.0  # if cam is not defined, use 45 deg (default value)
            else:
                fovy = self.model.cam_fovy[
                    0
                ]  # otherwise use the fovy of the first camera
        pcd, xyz_img, xyz_img_world = self.get_pcd_from_depth_img(
            depth_img_rsz, fovy=fovy
        )  # [N x 3]
    ...
    def get_fixed_cam_rgbd_pcd(self, cam_name, downscale_pcd=0.1):
        ...
        T_view = self.get_T_cam(cam_name=cam_name) @ pr2t(
            p=np.zeros(3), R=rpy2r(np.deg2rad([-45.0, 90.0, 45.0]))
        )
    ...
    def get_body_names(self, prefix="", excluding="world"):
        ...
        body_names = [
            x
            for x in self.body_names
            if x is not None and x.startswith(prefix) and excluding not in x
        ]
        return body_names
    ...
    def get_mesh_names(self, including="", excluding="collision"):
        ...
        if excluding is None:
            mesh_names = [
                x for x in self.mesh_names if x is not None and including in x
            ]
        else:
            mesh_names = [
                x
                for x in self.mesh_names
                if x is not None and including in x and excluding not in x
            ]
        return mesh_names
    ...
    def get_site_name_of_sensor(self, sensor_name):
        ...
        sensor_objtype = self.model.sensor_objtype[
            sensor_id
        ]  # get attached object type (i.e., site)
        sensor_objid = self.model.sensor_objid[sensor_id]  # get attached object ID
        site_name = mujoco.mj_id2name(
            self.model, sensor_objtype, sensor_objid
        )  # get the site name
        return site_name
    ...
    def get_sensor_values(self, sensor_names=None):
        ...
        data = np.array(
            [self.get_sensor_value(sensor_name) for sensor_name in self.sensor_names]
        ).squeeze()
        if self.n_sensor == 1:
            return [data]  # make it list
        else:
            return data.copy()
    ...
    def get_p_rf_obs_list(self, sensor_names):
        ...
        for sensor_name in sensor_names:  # for all sensors
            ...
            if cutoff_val == 0:
                cutoff_val = np.inf
            site_name = self.get_site_name_of_sensor(
    ...
    def plot_line(
        self, p=np.array([0, 0, 0]), R=np.eye(3), h=1.0, rgba=[0.5, 0.5, 0.5, 0.5]
    ):
        """
        Plot a line marker.
        ...
        """
        self.viewer.add_marker(
            pos=p, mat=R, type=mujoco.mjtGeom.mjGEOM_LINE, size=h, rgba=rgba, label=""
        )
    ...
    def plot_cylinder_fr2to(
        self, p_fr, p_to, r=0.01, rgba=[0.5, 0.5, 0.5, 0.5], label=""
    ):
        """
    ...
    def plot_traj(
        ...
        for idx in range(L - 1):
            ...
            p_to = traj[idx + 1, :]
            if len(p_fr) == 2:
                p_fr = np.append(p_fr, [0])
            if len(p_to) == 2:
                p_to = np.append(p_to, [0])
            if plot_line:
    ...
    def plot_time(
        ...
        self.viewer.add_overlay(
            text1="sim time", text2="%.2fsec" % (self.get_sim_time()), loc=loc
        )
        self.viewer.add_overlay(
            text1="wall time", text2="%.2fsec" % (self.get_wall_time()), loc=loc
        )
    ...
    def plot_links_between_bodies(
        ...
        """
        if pbne is not None:
            parent_body_names_to_exclude = pbne
        if bne is not None:
            body_names_to_exclude = bne
        for body_idx, body_name in enumerate(self.body_names):
            parent_body_name = self.parent_body_names[body_idx]
            if parent_body_name in parent_body_names_to_exclude:
                continue
            if body_name in body_names_to_exclude:
                continue
            if body_name is None:
    ...
    def plot_joint_axis(
        ...
        for rev_joint_idx, rev_joint_name in zip(
            rev_joint_idxs_to_use, rev_joint_names_to_use
        ):
    ...
    def get_contact_info(self, must_include_prefix=None, must_exclude_prefix=None):
        ...
        for c_idx in range(self.data.ncon):
            ...
            if must_include_prefix is not None:
                if (
                    contact_geom1[: len(must_include_prefix)] == must_include_prefix
                ) or (contact_geom2[: len(must_include_prefix)] == must_include_prefix):
                ...
                if (
                    contact_geom1[: len(must_exclude_prefix)] != must_exclude_prefix
                ) and (
    ...
    def print_contact_info(self, must_include_prefix=None):
        ...
        for p_contact, f_contact, geom1, geom2, body1, body2 in zip(
            p_contacts, f_contacts, geom1s, geom2s, body1s, body2s
        ):
            print("Tick:[%d] Body contact:[%s]-[%s]" % (self.tick, body1, body2))

    def plot_arrow_contact(
        self, p, uv, r_arrow=0.03, h_arrow=0.3, rgba=[1, 0, 0, 1], label=""
    ):
        """
    ...
    def plot_contact_info(
        ...
        for p_contact, f_contact, geom1, geom2, body1, body2 in zip(
            ...
            if plot_sphere:
                ...
                contact_label = ""
                self.plot_sphere(
                    p=p_contact, r=r_sphere, rgba=rgba_contact, label=contact_label
                )
    ...
    def get_qpos_joints(self, joint_names):
        ...
        """
        return np.array(
            [self.get_qpos_joint(joint_name) for joint_name in joint_names]
        ).squeeze()
    ...
    def get_qvel_joints(self, joint_names):
        ...
        """
        return np.array(
            [self.get_qvel_joint(joint_name) for joint_name in joint_names]
        ).squeeze()
        ...
        if coupled_joint_idxs_list is not None:
            for i in range(len(coupled_joint_idxs_list)):  # for each couple
                ...
                for k in range(len(coupled_joint_idxs)):
                    q_couple[coupled_joint_idxs[k]] = (
                        joint_sum * coupled_joint_weights[k]
                    )  # distribute coupled joint positions
    ...
    def get_viewer_mouse_xy(self):
        ...
        """
        viewer_mouse_xy = np.array(
            [self.viewer._last_mouse_x, self.viewer._last_mouse_y]
        )
    ...
    def get_xyz_left_double_click(self, verbose=False, fovy=45):
        ...
        if self.viewer._left_double_click_pressed:  # left double click
            ...
            self.xyz_left_double_click = xyz_img_world[
                int(viewer_mouse_xy[1]), int(viewer_mouse_xy[0])
            ]
            self.viewer._left_double_click_pressed = False
            flag_click = True
            if verbose:
                print(
                    "left double click:(%.3f,%.3f,%.3f)"
                    % (
                        self.xyz_left_double_click[0],
                        self.xyz_left_double_click[1],
                        self.xyz_left_double_click[2],
                    )
                )
        return self.xyz_left_double_click, flag_click
    ...
    def get_xyz_right_double_click(self, verbose=False, fovy=45):
        ...
        if self.viewer._right_double_click_pressed:  # right double click
            viewer_mouse_xy = self.get_viewer_mouse_xy()
            _, _, _, _, xyz_img_world = self.get_egocentric_rgbd_pcd(fovy=fovy)
            self.xyz_right_double_click = xyz_img_world[
                int(viewer_mouse_xy[1]), int(viewer_mouse_xy[0])
            ]
            ...
            if verbose:
                print(
                    "right double click:(%.3f,%.3f,%.3f)"
                    % (
                        self.xyz_right_double_click[0],
                        self.xyz_right_double_click[1],
                        self.xyz_right_double_click[2],
                    )
                )
    ...
    def get_ik_ingredients(
        ...
        if (body_name is not None) and (geom_name is not None):
            print(
                "[get_ik_ingredients] body_name:[%s] geom_name:[%s] are both not None!"
                % (body_name, geom_name)
            )
        if IK_P and IK_R:
            p_err = p_trgt - p_curr
