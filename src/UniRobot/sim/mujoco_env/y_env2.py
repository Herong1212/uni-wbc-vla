...
from UniRobot.sim.mujoco_env.mujoco_parser import MuJoCoParserClass
from UniRobot.sim.mujoco_env.utils import (
    prettify,
    sample_xyzs,
    rotation_matrix,
    add_title_to_img,
)
from UniRobot.sim.mujoco_env.ik import solve_ik
from UniRobot.sim.mujoco_env.transforms import rpy2r, r2rpy
import os
import copy
import glfw
...
class SimpleEnv2:
    ...
    def reset(self, seed=None):
        ...
        self.obj_init_pose = np.concatenate(
            [mug_red_init_pose, mug_blue_init_pose, plate_init_pose], dtype=np.float32
        )
        for _ in range(100):
            self.step_env()
    ...
    def set_instruction(self, given=None):
        ...
        if given is None:
            ...
            if "red" in self.instruction:
                ...
                raise ValueError(
                    "Instruction does not contain a valid object color (red or blue)."
                )
    ...
    def render(self, teleop=False, idx=0):
        ...
        if getattr(self, "instruction", None) is not None:
            ...
            self.env.viewer_text_overlay(
                text1="Language Instructions", text2=language_instructions
            )
        self.env.render()
